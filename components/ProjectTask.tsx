        ...
        <div>
            ...
            <div className="flex flex-wrap gap-4 mb-4">
                {["status", "type", "priority", "assignee"].map((name) => {
                    ...
                    return (
                        <Select key={name} defaultValue={options[name][0].value} name={name} onValueChange={(val) => handleFilterChange(name, val)}>
                            <SelectTrigger id={name}>
                                <SelectValue defaultValue={options[name][0].value} />
                            </SelectTrigger>
                            <SelectContent>
                                <SelectGroup>
                                    {options[name].map((opt:any, idx:any) => (
                ...
                {(filters.status || filters.type || filters.priority || filters.assignee) && (
                    <Button variant="default" onClick={() => setFilters({ status: "", type: "", priority: "", assignee: "" })} className="px-3 py-1 flex items-center gap-2 rounded bg-linear-to-br from-purple-400 to-purple-500 text-zinc-100 dark:text-zinc-200 text-sm transition-colors" >
                        <XIcon className="size-3" /> Reset
                    </Button>
                )}

                {selectedTasks.length > 0 && (
                    <Button variant="default" type="button" onClick={handleDelete} className="px-3 py-1 flex items-center gap-2 rounded bg-linear-to-br from-indigo-400 to-indigo-500 text-zinc-100 dark:text-zinc-200 text-sm transition-colors" >
                            ...
                            <thead className="text-xs uppercase dark:bg-zinc-800/70 text-zinc-500 dark:text-zinc-400 ">
                                <tr>
                                    <th className="pl-2 pr-1">
                                        <input
                                        onChange={() => 
                                            selectedTasks.length > 1 ? setSelectedTasks([]) : setSelectedTasks(tasks.map((t) => t.id))} 
                                        ...
                                        return (
                                            <tr key={task.id} onClick={() => router.push(`/taskDetails?projectId=${task.projectId}&taskId=${task.id}`)} className=" border-t border-zinc-300 dark:border-zinc-800 group hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-all cursor-pointer" >
                                                <td onClick={e => e.stopPropagation()} className="pl-2 pr-1">
                                                    <input type="checkbox" className="size-3 accent-zinc-600 dark:accent-zinc-500" 
                                                        onChange={() => 
                                                        selectedTasks.includes(task.id as never) ? setSelectedTasks(selectedTasks.filter((i) => i !== task.id)) : setSelectedTasks((prev) => [...prev, task.id])} 
                                                        checked={selectedTasks.includes(task.id as never)} />
                                                </td>
                                                ...
                                                <td onClick={e => e.stopPropagation()} className="px-4 py-2">
                                                    <Select defaultValue={task.status} onValueChange={(val) => handleStatusChange(task.id, val as TaskStatus)}>
                                                        <SelectTrigger>
                                                ...
                                                <td className="px-4 py-2">
                                                    <div className="flex items-center gap-2">
                                                        <Image width={20} height={20} src={task.assignee?.image} className="size-5 rounded-full" alt="avatar" />
                                                        {task.assignee?.name || "-"}
                                                    </div>
                                                </td>
                                                <td className="px-4 py-2">
                                ...
                                return (
                                    <div key={task.id} className=" dark:bg-linear-to-br dark:from-zinc-800/70 dark:to-zinc-900/50 border border-zinc-300 dark:border-zinc-800 rounded-lg p-4 flex flex-col gap-2">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <input type="checkbox" className="size-4 accent-zinc-600 dark:accent-zinc-500" 
                                                onChange={() => selectedTasks.includes(task.id as never) ? setSelectedTasks(selectedTasks.filter((i) => i !== task.id)) : setSelectedTasks((prev) => [...prev, task.id])} 
                                                checked={selectedTasks.includes(task.id as never)} 
                                            />
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <select name="status" 
                                                ...
                                                value={task.status} className="w-full mt-1 bg-zinc-100 dark:bg-zinc-800 ring-1 ring-zinc-300 dark:ring-zinc-700 outline-none px-2 py-1 rounded text-sm text-zinc-900 dark:text-zinc-200" 
                                            >
