            ...
            <SidebarGroupLabel asChild>
              <CollapsibleTrigger className="hover:bg-gray-50 dark:hover:bg-zinc-800/60 p-2">
                <div className="flex items-center gap-3">
                  ...
                  <span className="bg-gray-200 dark:bg-zinc-700 text-gray-700 dark:text-zinc-300 text-xs px-2 py-0.5 rounded">{tasks.length}</span>
                </div>
              ...
              <SidebarGroupContent>
                {
                  tasks.slice(0,3).map((t) => (
                    <Link key={t.title} href={`/task-detail?taskId=${t.id}&projectId=${t.projectId}`} className="w-full rounded-lg transition-all duration-200 text-gray-700 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800 hover:text-black dark:hover:text-white" >
                        <div className="flex items-center gap-2 px-3 py-2 w-full min-w-0">
                            <div className={`w-2 h-2 rounded-full ${getTaskStatusColor(t.status)} shrink-0`} />
