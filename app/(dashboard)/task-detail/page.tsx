...
import { dummyUsers, dummyWorkspaces } from "@/public/assets/dummyData";
...
const TaskDetail = () => {
    ...
    const handleAddComment = async () => {
        ...
        try {
            ...
            const dummyComment :Comment = { id: '490hf9c232', userId: user.id, taskId: currentWorkspace.projects[0].tasks[0].id , message: newComment, createdAt: "10-03-2026" };
                        ...
                        {comments.length > 0 ? (
                            <div className="flex flex-col gap-4 mb-6 mr-2">
                                {comments.map((comment) => (
                                    <div key={comment.id} className={`sm:max-w-4/5 dark:bg-linear-to-br dark:from-zinc-800 dark:to-zinc-900 border border-gray-300 dark:border-zinc-700 p-3 rounded-md ${user.id === user?.id ? "ml-auto" : "mr-auto"}`} >
                                        <div className="flex items-center gap-2 mb-1 text-sm text-gray-500 dark:text-zinc-400">
                                            <Image width={20} height={20} src={user?.image} alt="avatar" className="size-5 rounded-full" />
                                            <span className="font-medium text-gray-900 dark:text-white">{user.name}</span>
                                            <span className="text-xs text-gray-400 dark:text-zinc-600">
                                                • {format(new Date(comment.createdAt), "dd MMM yyyy, HH:mm")}
                                            </span>
                                        </div>
                                        <p className="text-sm text-gray-900 dark:text-zinc-200">{comment.message}</p>
                                    </div>
                                ))}
                            </div>
                        ) : (
                ...
                <div className="p-5 rounded-md bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-800 ">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-700 dark:text-zinc-300">
                        <div className="flex items-center gap-2">
                            <Image width={20} height={20} src={task?.assignee?.image as string} className="size-5 rounded-full" alt="avatar" />
